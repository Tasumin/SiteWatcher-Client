...
[pscustomobject]@{installed=[bool]($svc -or $exe);serviceName=if($svc){$svc.Name}else{$null};serviceStatus=if($svc){[string]$svc.Status}else{'Missing'};startType=if($svc){[string]$svc.StartType}else{$null};exe=$exe;version=$ver;listening=($l.Count-gt 0);listeners=$l;port=5900;passwordRequired=$false;loopbackOnly=$true}|ConvertTo-Json -Depth 5 -Compress
"""
    c=_ps(script,30)
...
def install_tightvnc() -> dict:
    ...
    if existing.get("installed"):
        ...
        return existing
    temp_dir=tempfile.gettempdir(); msi=os.path.join(temp_dir,f"tightvnc-{TIGHTVNC_VERSION}-64bit.msi"); msi_log=os.path.join(temp_dir,"sitewatcher-tightvnc-install.log")
    try:
        urllib.request.urlretrieve(TIGHTVNC_MSI_URL,msi)
        if not os.path.exists(msi) or os.path.getsize(msi)<100000: raise RuntimeError("Downloaded TightVNC MSI is missing or unexpectedly small.")
        # Server/service install. Deliberately passwordless, but restricted to local loopback so VNC is reachable only through SiteWatcher's tunnel.
        args=["msiexec.exe","/i",msi,"/quiet","/norestart","/L*v",msi_log,"ADDLOCAL=Server","SERVER_REGISTER_AS_SERVICE=1","SERVER_ADD_FIREWALL_EXCEPTION=0","SET_ACCEPTRFBCONNECTIONS=1","VALUE_OF_ACCEPTRFBCONNECTIONS=1","SET_ALLOWLOOPBACK=1","VALUE_OF_ALLOWLOOPBACK=1","SET_LOOPBACKONLY=1","VALUE_OF_LOOPBACKONLY=1","SET_RFBPORT=1","VALUE_OF_RFBPORT=5900","SET_USEVNCAUTHENTICATION=1","VALUE_OF_USEVNCAUTHENTICATION=0","SET_USECONTROLAUTHENTICATION=1","VALUE_OF_USECONTROLAUTHENTICATION=0"]
        c=subprocess.run(args,capture_output=True,text=True,errors="replace",timeout=240,creationflags=CREATE_NO_WINDOW)
        if c.returncode not in (0,3010):
            tail=""
            for enc in ("utf-16","utf-8"):
                try:
                    with open(msi_log,"r",encoding=enc,errors="replace") as h: tail="\n".join(h.read().splitlines()[-40:]); break
                except Exception: pass
            raise RuntimeError(f"TightVNC MSI exited with code {c.returncode}. {(c.stderr or c.stdout or '').strip()}\n{tail}".strip())
        ...
        status=get_tightvnc_status(); status["restartRequired"]=c.returncode==3010; status["installerVersion"]=TIGHTVNC_VERSION
        if not status.get("installed"): raise RuntimeError("TightVNC MSI completed successfully, but the TightVNC service/executable was not detected afterward.")
        return status
    finally:
        try: os.remove(msi)
        except OSError: pass
...
def uninstall_tightvnc() -> dict:
    script=r"""$uninstall=@('HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*','HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*')|%{Get-ItemProperty $_ -ErrorAction SilentlyContinue}|?{$_.DisplayName -like 'TightVNC*'}|Select-Object -First 1;if(-not$uninstall){throw 'TightVNC is not installed.'};$p=Start-Process msiexec.exe -ArgumentList @('/x',$uninstall.PSChildName,'/quiet','/norestart') -Wait -PassThru;if($p.ExitCode -notin 0,3010){throw "TightVNC uninstall exited with code $($p.ExitCode)"}"""
    c=_ps(script,180)
